...
void handleWEFAC(HandlerContext& handlerContext)
{
    for (const auto& record : handlerContext.keyword) {
        ...
        const bool useEfficiencyInExtNetwork = DeckItem::to_bool(record.getItem("EXTENDED_NETWORK_OPT").getTrimmedString(0));

        for (const auto& well_name : well_names) {
            auto well2 = handlerContext.state().wells.get( well_name );
            if (well2.updateEfficiencyFactor(efficiencyFactor, useEfficiencyInExtNetwork)){
