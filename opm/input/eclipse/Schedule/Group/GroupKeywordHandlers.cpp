...
void handleGEFAC(HandlerContext& handlerContext)
{
    for (const auto& record : handlerContext.keyword) {
        ...
        const bool use_efficiency_in_ext_network = DeckItem::to_bool(record.getItem("TRANSFER_EXT_NET").getTrimmedString(0));
        const auto gefac = record.getItem("EFFICIENCY_FACTOR").get<double>(0);

        for (const auto& group_name : group_names) {
            auto new_group = handlerContext.state().groups.get(group_name);
            if (new_group.update_gefac(gefac, use_efficiency_in_ext_network)) {
