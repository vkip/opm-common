...
std::pair<Opm::Deck, Opm::Deck> load_deck(const Options& opt)
{
    ...
    return { Opm::Parser{}.parseFile(opt.input_deck, parseContext, errors),
             Opm::Parser{}.parseFile(opt.insert_deck, parseContext, errors)};
}
...
std::pair<Options, std::string> load_options(int argc, char **argv)
{
    ...
    opt.insert_deck = argv[arg_offset+1];
    const std::string restart_arg = argv[arg_offset + 2];
    if ((argc - arg_offset) >= 4) {
        const auto target_arg = argv[arg_offset + 3];
...
void insert_schedule_deck(const Options& opt, const Opm::Deck& insert_deck, Opm::FileDeck& input_file_deck)
{
    const auto report_step = opt.restart_step;

    int current_report = 0;
    const auto& schedule = input_file_deck.find("SCHEDULE");
    auto deck_pos = schedule.value();
    while (true) {
        const auto& deck_keyword = input_file_deck[deck_pos];
        if (deck_keyword.name() == "DATES") {
            current_report += deck_keyword.size();
        }
        else if (deck_keyword.name() == "TSTEP") {
            current_report += deck_keyword[0]
                .getItem<Opm::ParserKeywords::TSTEP::step_list>()
                .data_size();
        }

        if (current_report > report_step || deck_keyword.name() == "END") {
            break;
        }

        if (deck_keyword.name() == "END") {
            --deck_pos;
            std::cout << fmt::format("Warning: Found 'END' before given restart step = {}, inserting here...", report_step) << std::endl;
            break;
        }

        ++deck_pos;
        if (deck_pos == input_file_deck.stop()) {
            std::cout << fmt::format("Warning: End of deck before given restart step = {}, inserting here...", report_step) << std::endl;
            --deck_pos;
            break;
        }
    }
    for (auto kw : insert_deck) {
        input_file_deck.insert(deck_pos, kw);
        ++deck_pos;
    }
}

void update_schedule(const Options& opt, Opm::FileDeck& input_file_deck)
{
    if (opt.restart_step == 0) {
        return;
    }
    ...
    if (opt.skiprest) {
        input_file_deck.insert_skiprest();
    }
    else {
        input_file_deck.skip(opt.restart_step);
    }
}
...
int main(int argc, char** argv)
{
    ...
    const auto [input_deck, insert_deck] = load_deck(options);
    Opm::FileDeck input_file_deck(input_deck);


    update_restart_path(options, restart_arg, Opm::IOConfig(input_deck));
    update_solution(options, input_file_deck);
    insert_schedule_deck(options, insert_deck, input_file_deck);
    update_schedule(options, input_file_deck);

    if (!options.target_path.has_value()) {
        input_file_deck.dump_stdout(fs::current_path(), options.mode);
    }
    else {
        input_file_deck.dump( options.target_path.value(), options.target_fname.value(), options.mode);
    }
}
